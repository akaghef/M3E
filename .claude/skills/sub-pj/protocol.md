...
- `facet type`: facet の汎用パターン。`flow` / `dependency` / `reviews` など
- `view`: runtime 上で人間に見せる画面構成。facet を前景化した実行面
- `board`: 要約 view。summary でも traceability を必須とする
- `sprint contract`: 実装前に固定する task の終了条件。`done_when` / `eval_criteria` / `round_max` を含む
- `Generator`: 実装・詳細化担当の subagent
- `Evaluator`: `done_when` と品質観点を独立に検証する subagent

完全版は `references/glossary.md` を参照。
...
2. **ビジョン凝縮**（ユーザーと対話）
   ...
   - 半年後の完了像
   - 明示的に範囲外とするもの
   - ブレスト素材（docs/ideas/ 等）があっても PJ スコープとは別物。ここで絞る
3. **ディレクトリ作成** — `projects/PJ{NN}_{Name}/` に以下のサブディレクトリも作成:
   - `docs/ideas/` — PJ 内で生まれたアイデア・メモ
   - `data/` — 入力データ・中間データ
   - `docs/` — 情報源・参考文献・調査メモ
...
人間不在でも inner loop を回す PJ は、上の 3-view runtime を壊さずに
**Evaluation Board** を追加した 4-view 変種を持ってよい。

| view | 主目的 |
|---|---|
| **Progress Board** | task / phase / blocker / next task |
| **Evaluation Board** | Evaluator verdict, `done_when` 検証結果, round feedback |
| **Review** | 人間の outer-loop 修正。方向性・quality bar・phase gate |
| **Active Workspace** | 実装・設計・根拠へ drill-down |

原則:

- inner loop は outer loop を待たずに進む
- outer loop は board を見て軌道修正する
- `Review` は人間チャネル、`Evaluation Board` は機械検証チャネルとして分ける
- map から chat に送れない場合でも board は更新し続ける

### plan -> do handoff 原則

`do` を始める前に、`plan` 側で次を済ませておくこと:

- 使う facet が確定している
- 各 facet の runtime 上の置き場（scope / anchor / exemplar）が初期化済み
- Progress Board / Review / Active Workspace / Evaluation Board の有無が決まっている
- task から facet / workspace への traceability の雛形がある

`do` は map 構造を発明しない。`plan` が用意した器に対して書き込む。
...
| **アイデア・発見** | PJ内 docs/ideas/ or backlog/（tomd） | 作業中に思いついた時。PJ スコープ外 |
...
### 6.5 tasks.yaml — sprint contract 正本

自走ハーネスを使う PJ では、`projects/PJ{NN}_{Name}/tasks.yaml` を
task 実行の正本とする。plan.md の task 箇条書きではなく、以下の構造体で管理する。

```yaml
- id: T-0-1
  phase: 0
  verb: add
  target: "edgeType kind 語彙定義"
  done_when:
    - "relationType enum に 7 種追加"
    - "npm run build pass"
  eval_required: true
  eval_criteria:
    - "naming convention が既存 enum と整合"
    - "edge case を見落としていない"
...
- `id`: `T-{phase}-{seq}` 形式
- `phase`: 属する Phase 番号
- `verb` / `target`: 1 行で task を説明する最小単位
- `done_when`: objective な終了条件
- `eval_required`: Evaluator を起動するか
- `eval_criteria`: `done_when` を超えた品質観点
- `status`: `pending | in_progress | done | blocked`
- `round`: 現在の round 数
- `round_max`: 再試行上限
- `last_feedback`: Evaluator からの最新 feedback
...
- Generator 起動時に `pending -> in_progress`
- Evaluator `pass` で `done`
- Evaluator `fail` で `round += 1`
- `round > round_max` または外部要因停止で `blocked`

#### 運用原則

- `done_when` は todo ではなく sprint contract である。実装開始後に勝手に動かすな
- Manager は quality judgment をしない。`eval_required: true` の task は Evaluator verdict を採用する
- `eval_required: false` の task だけ、Manager が軽量 objective check をしてよい
- blocked 理由や未確定は `reviews/` に pool し、可能なら tentative default で進める

### 6.6 resume-cheatsheet.md — 再開正本

`projects/PJ{NN}_{Name}/resume-cheatsheet.md` は、compact やセッション再開時の 1 画面復元用。
含める項目:
...
`resume.md` と hook scripts は、このファイルを最優先で読む。

### 6.7 do 開始条件

`sub-pj-do` を始めてよいのは、少なくとも次が揃った後:

- `tasks.yaml` が存在する
- `resume-cheatsheet.md` が存在する
- 担当 facet / scope が map 上に存在する
- 作業先の facet が `tasks.yaml` の各 task に書かれている

足りない場合は `do` ではなく `plan` の不足として扱い、先に bootstrap をやり直す。
