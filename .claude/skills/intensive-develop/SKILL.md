...
1. **mvp/ は完全無視** — 読まない、書かない、参照しない
2. **beta/ が開発対象** — コード変更は beta/ 配下のみ
3. **final/ は触らない** — launch-final skill の管轄
4. **破壊的操作は自動実行しない** — force push, reset --hard, ブランチ削除は人間の確認が必要
5. **アサインの上書きは確認を求める** — 既に assigned/doing のタスクを別ワーカーに振り直す場合はユーザーに���く
6. **idle tick はトークンを使わない** — Step 2 の早期終了を厳守
7. **1 tick あたりのサブエージェント起動は最大2つ** — リソース制御
