...
- daily note（`docs/daily/YYMMDD.md`）が更新されているか
...
統合ロールとして `docs/00_Home/Current_Status.md` を更新:
- 「最近の成果」に追加
...
`docs/06_Operations/Todo_Pool.md` の該当項目を `done` に変更。
...
feature branch（ロールブランチ以外の一時ブランチ）の場合:
```bash
gh pr close <number> --delete-branch
```

ロールブランチ（`dev-visual` / `dev-data` / `dev-data2` / `dev-team`）は永続ブランチなので削除しない。
