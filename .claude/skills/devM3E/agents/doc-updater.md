...
- `dev-docs/daily/YYMMDD.md` は **バックアップ兼 git 履歴用** として残すが、正本は M3E ドキュメント
...
同時に `dev-docs/daily/{YYMMDD}.md` にも同じ内容を追記する（git バックアップ）。
...
サーバー未起動時は `dev-docs/daily/{YYMMDD}.md` にのみ記録し、次回起動時にマップ反映する。
...
`dev-docs/06_Operations/Todo_Pool.md` への追記は **pooled（未整理）** と **blocked（判断待ち）** のみ。
