...
beta と final で起動ポートが異なる。同時起動を許容するための措置。

| 環境 | ポート | 設定箇所 |
|------|--------|---------|
| beta | 4173 | Vite デフォルト（`vite preview --port 4173`） |
| final | **38482** | `scripts/final/launch.bat`, `launch.sh` で `M3E_PORT=38482` |

環境変数 `M3E_PORT` で上書き可能。

## リリースタグ

main へのマージ時に `vYYMMDD` 形式のタグを付与する。
同日に複数回リリースする場合は `-2`, `-3`, ... のサフィックスを追加。

例: `v260402`, `v260402-2`, `v260403`, `v260403-2`, `v260403-3`

## 運用スクリプト

| スクリプト | 用途 |
|-----------|------|
| `scripts/final/launch.bat` / `.sh` | Final 起動（M3E_DATA_DIR 設定込み） |
| `scripts/final/launch-hidden.vbs` | バックグラウンド起動（Windows） |
| `scripts/final/migrate-from-beta.bat` / `.sh` | Beta → Final フル同期 + ビルド + 起動 |
| `scripts/final/update-and-launch.bat` / `.sh` | pull + 同期 + ビルド + 起動（日常運用） |
