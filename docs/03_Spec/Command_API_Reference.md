# M3E Command API Reference

書き平面の実装リファレンス。平面構造と語彙は [Command_Language.md](./Command_Language.md) が正であり、この文書は既存 beta UI / browser console の `window.m3e` 互換 API について、シグネチャ・引数・戻り値・エラー仕様・組み合わせ例を保持する。
