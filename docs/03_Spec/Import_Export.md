...
- M3E-native Rapid document の論理正本は **SavedDoc JSON**（`version: 1`）とする
- この import / export 操作へ取り込んだ copy は M3E 形式へ変換する。live binding または federated source の正本割当は本仕様の対象外とする
- ラウンドトリップ（M3E → 外部 → M3E）で意味が保たれることを重視する
- 外部フォーマットで表現できない M3E 固有の概念（scope・alias 等）は、エクスポート時に落とすか属性として退避する

Git / Obsidian / external provider を canonical source として接続する場合は [Federated_Semantic_Source.md](./Federated_Semantic_Source.md) に従い、単発 import / export と混同しない。
...
M3E-native Rapid document の論理正本フォーマット。ファイル保存・ブラウザダウンロード・SQLite 内部保存に使用する。
...
- Freeplane マッピング方針: [../competitive_research/freeplane/Freeplane_Data_Model_Mapping.md](../competitive_research/freeplane/Freeplane_Data_Model_Mapping.md)
