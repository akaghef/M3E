...
本仕様の「正本性、採否、復元責任を M3E 側に残す」は、今回の V4 demo workspace と M3E 固有 semantics に適用する。S16 の federated source では provider-native state は provider、repository code / document は Git、M3E 固有 relation / approval は M3E が所有する。M3E がすべての外部本文を奪う意味ではない。詳細は [Federated_Semantic_Source.md](./Federated_Semantic_Source.md) に従う。

今回の縦スライスは、Swingby 公開環境 `ws_team_swingby` 上で次を実行済みの最小実装として扱う。
ただし、これは API レベルの成立確認であり、プロダクト体験としての到達点は
[V4_Product_Experience_Blueprint.md](./V4_Product_Experience_Blueprint.md) を正本にする。
...
Standalone M3E Rapid では SQLite workspace を使うが、明示的な local-file binding mode では vault の `.md` を canonical source とする。
...
- 今回の V4 demo workspace は、Akaghef PC 上の local workspace を canonical owner として運用する。
- Git-backed source を採用した repository では、Git repository が code / design document / repository-local semantic assertion の canonical source になりうる。GitHub remote はその source の配布、履歴、レビュー、復元補助を担う。
