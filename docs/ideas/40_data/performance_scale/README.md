...
- `docs/ideas/cross_device/` — Pf1 PWA / Sy1-Sy10 同期と本ブレストの Ch1-Ch12 が重複
- `docs/ideas/maintenance_hygiene/` — 重複検出（D1）が走る基盤として検索インデックスが必要
- `docs/ideas/time_history/` — Sr10 の history インデックスは I1 time travel と直結
- `docs/ideas/automation_obstacles/` — workspace 横断ナビは自動化の限界点を含む
- `docs/ideas/00_topic_pool.md` — M セクション（M1-M5）が本ブレストの起点
