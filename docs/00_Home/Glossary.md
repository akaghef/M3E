...
| 正規語 | 意味 | 別表記 / 禁止語 | 備考 |
|---|---|---|---|
| **facet** | PJ の一側面を切り出した scope であり、かつその切り出しが**意味単位として凝集している**もの。両方の条件を満たす場合のみ facet と呼ぶ | （禁止）「12 スコープ」「観点スコープ」 | **PJ 直下の第一階層に限定**。facet 内部のサブツリー（例: `src/core/`）は「小分類」として扱い、facet には含めない。実体は 1 箇所、他 facet からは alias で参照 |
...
| **小分類** | facet 内部の整理用サブツリー（例: `src/core/`, `src/io/`） | - | ただの scope として扱う。facet 要件（PJ 一側面 + 凝集）は課さない |
