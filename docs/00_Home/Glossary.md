...
| 正規語 | 意味 | 別表記 / 禁止語 | 備考 |
|---|---|---|---|
| **Flash** | 断片・素材の領域。マルチモーダル（テキスト/画像/音声/貼付）、日常との連結、突発・散発的アイデア。構造化前 | inbox (Flash 内の受け皿名) | |
| **Rapid** | **文書 1 つ単位**の領域。syntax tree（親子・章立て・節、説明順序あり、線形化可能） | - | 現在の実装中心。Freeplane mindmap / LaTeX 章立て的 |
| **Deep** | **文書群・知識体系**の領域。semantic graph（多種エッジ、説明順序と独立、同ノードが複数経路に現れる） | - | nLab / Wikidata / mathlib4 dep 網的 |
| **昇格 (promote)** | Flash → Rapid への統合操作 | - | |
| **体系化** | **Rapid → Deep** への変換操作。複数文書を束ねて概念網に編み上げる | - | 射影の逆方向 |
| **射影 (projection)** | **Deep → Rapid** への変換操作。体系から特定目的の説明順序を持つ 1 文書を切り出す | - | 同じ Deep から科研費/学振/JST 等を別々に射影。世界モデル=資産、射影出力=使い捨て。`memory/project_projection_vision.md` 参照 |
| **syntax tree** | 親子・線形化可能・説明順序ありの木構造。Rapid の内部構造 | - | 本文・章立て・Freeplane mindmap 的 |
| **semantic graph** | 多種エッジ・説明順序非依存の網構造。Deep の内部構造 | semantic tree (akaghef 用法、ただし tree ではなく graph) | 一般化/双対/類比/動機/類似 等の関係種別 |
| **世界モデル** | Deep の到達形。射影の源泉となる資産 | - | |
