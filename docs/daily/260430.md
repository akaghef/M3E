...
## PJ04 Mermaid Preview Index

- `template_mermaid_cli.ts` を `--spec-dir` 対応にし、`projects/PJ04_MermaidSystemLangGraph/templates/` 配下の全 Template System Spec を 1 つの read-only Mermaid index HTML に一覧化できるようにした。
- `npm run template:mermaid:all` を追加し、`tmp/pj04-mermaid-index.html` に PJv34 / PJv35 の root + subsystem 計 4 diagram を出力する。
- 検証: `npm run template:test` は 8 tests pass。Playwright で index HTML を開き、4 section / 4 nav item / 4 SVG、syntax error なし、PJv34 / PJv35 の主要 label 表示を確認した。

## PJ04 PJv Mermaid Review Board

- PJv36 / PJv37 の assignment spec と Template System Spec、PJv43 の Template System Spec を追加し、PJv34 / PJv35 / PJv36 / PJv37 / PJv43 を同じ Mermaid preview index に載せた。
- Mermaid index に LangGraph System Block Template の status table を追加し、registry 上の 14 template について `USED` / `READY`、使用 PJv、required slots を一覧できるようにした。
- 各 diagram section に `Accept` / `Reject` feedback button を追加した。現時点では static HTML の `localStorage` に保存する review aid とし、map 正本や docs への自動反映はしない。
- 検証: `npm run template:test` は 8 tests pass、`npm run template:mermaid:all` は PJv34 / PJv35 / PJv36 / PJv37 / PJv43 から 10 diagrams を生成。Playwright で 10 section / 10 nav item / 10 SVG / 14 template rows / 20 feedback buttons、syntax error なし、PJv36 / PJv37 / PJv43 label と feedback accept/reject 動作を確認した。
