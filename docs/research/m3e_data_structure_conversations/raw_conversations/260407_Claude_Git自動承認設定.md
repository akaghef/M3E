---
title: "Claude Git自動承認設定"
created: 2026-04-07 16:55:43
updated: 2026-04-07 16:56:49
tags:
  - chatgpt
  - conversation
---
...
**User**: <sub>2026-04-07 16:55</sub>

claudeがめちゃくちゃgit, cdを使っていいか聞いてくるんだが、どうやって自動で承認させる？

> [!abstract]- Tool (file_search)
> <sub>2026-04-07 16:55</sub>
> 
...
**ChatGPT (tool execution)**: <sub>2026-04-07 16:55</sub>

```
{"search_query":[{"q":"Anthropic Claude Code auto approve permissions bash commands git cd allowlist official docs","domains":["docs.anthropic.com","support.anthropic.com"]},{"q":"site:docs.anthropic.com Claude Code permissions allowlist git cd","domains":["docs.anthropic.com"]}],"response_length":"short"}
```

**ChatGPT (tool execution)**: <sub>2026-04-07 16:55</sub>

```
{"open":[{"ref_id":"turn854067search1"}],"response_length":"short"}
```

**ChatGPT (tool execution)**: <sub>2026-04-07 16:55</sub>

```
{"find":[{"ref_id":"turn145073view0","pattern":"permissions"},{"ref_id":"turn145073view0","pattern":"Bash(git"},{"ref_id":"turn145073view0","pattern":"Bash(cd"}],"response_length":"short"}
```

> [!note]- Internal Reasoning
> <sub>2026-04-07 16:56</sub>
> 
> **cdコマンドと権限について考察**: cdコマンドはシェルにおける組み込みコマンドで、権限が依存することを説明する必要があります。もし`cd *`のような使い方ができるなら、複数回の`cd`を使った場合、動作がスムーズに行くかどうか検討すべきかもしれません。また、`/permissions`について触れるのも良さそうです。

**ChatGPT (tool execution)**: <sub>2026-04-07 16:55</sub>

```
{"search_query":[{"q":"site:code.claude.com/docs /permissions Claude Code official"},{"q":"site:docs.anthropic.com/en/docs/claude-code /permissions command Claude Code official"}],"response_length":"short"}
```
...
**ChatGPT**: <sub>2026-04-07 16:56</sub>

Claude Code なら、まず `/permissions` で許可ルールを追加するのが正攻法です。`allow` は確認なしで実行、`ask` は毎回確認、`deny` は拒否です。ルールは `deny -> ask -> allow` の順で評価されます。設定は `~/.claude/settings.json`、`.claude/settings.json`、`.claude/settings.local.json` に置けます。citeturn491213search0turn726273view0turn145073view0

たとえば、自分だけに効かせるなら `~/.claude/settings.json` にこう入れます。
...
Claude Code の公式設定例でも `permissions.allow` に `Bash(...)` を並べる形式が使われています。`git diff` や `git push` のように、コマンド単位で細かく制御できます。citeturn145073view0turn726273view0

`git push` だけは確認を残したいなら、こう分けます。
...
公式にも `allow` と `ask` の両方があり、`ask` の例として `Bash(git push *)` が示されています。citeturn726273view0

もっと強く自動化するなら、`defaultMode` を `auto`、`dontAsk`、`bypassPermissions` のいずれかにできます。`bypassPermissions` はほぼ全確認を飛ばしますが、危険なので通常は勧めません。`--permission-mode` でも単発セッションだけ上書きできます。citeturn726273view0turn491213search2turn491213search12

要点だけ言うと、普段は `/permissions` で `Bash(git ... )` を許可し、`push` だけ `ask` に残すのが実用的です。全部飛ばしたいなら `bypassPermissions` ですが、誤操作のリスクが上がります。citeturn491213search0turn726273view0turn491213search10
