---
title: "数学研究と著作権"
created: 2026-04-22 23:58:51
updated: 2026-04-23 00:02:07
tags:
  - chatgpt
  - conversation
---
...
**ChatGPT**: <sub>2026-04-22 23:58</sub>

論点を整理して、著作権法と契約を分けて見ます。日本法ベースで、研究設計として安全な線と危ない線をケースごとに切ります。

**ChatGPT (tool execution)**: <sub>2026-04-22 23:58</sub>

```
{"search_query":[{"q":"日本 著作権法 30条の4 情報解析 e-Gov","domains":["elaws.e-gov.go.jp","www.bunka.go.jp"]},{"q":"文化庁 著作権Q&A AI 情報解析 30条の4","domains":["www.bunka.go.jp"]},{"q":"日本 著作権法 47条の5 電子計算機による情報処理 e-Gov","domains":["elaws.e-gov.go.jp","www.bunka.go.jp"]},{"q":"文化庁 AIと著作権 考え方 2024","domains":["www.bunka.go.jp"]}],"response_length":"short"}
```

**ChatGPT (tool execution)**: <sub>2026-04-22 23:59</sub>

```
{"search_query":[{"q":"e-Gov 著作権法 第30条の4","domains":["elaws.e-gov.go.jp"]},{"q":"e-Gov 著作権法 第47条の5","domains":["elaws.e-gov.go.jp"]},{"q":"e-Gov 著作権法 第47条の4","domains":["elaws.e-gov.go.jp"]},{"q":"e-Gov 著作権法 第113条 みなし侵害 送信 侵害","domains":["elaws.e-gov.go.jp"]}],"response_length":"short"}
```

**ChatGPT (tool execution)**: <sub>2026-04-22 23:59</sub>

```
{"search_query":[{"q":"site:bunka.go.jp RAG 30条の4 47条の5 AIと著作権","domains":["www.bunka.go.jp"]},{"q":"site:bunka.go.jp 著作権 契約 利用規約 AI 機械学習 電子書籍","domains":["www.bunka.go.jp"]},{"q":"site:bunka.go.jp 著作権法 30条の4 ただし書き 市場 侵害 AI","domains":["www.bunka.go.jp"]},{"q":"site:bunka.go.jp AIと著作権 チェックリスト 契約 AI利用者 業務利用者","domains":["www.bunka.go.jp"]}],"response_length":"short"}
```

> [!note]- Internal Reasoning
> <sub>2026-04-22 23:59</sub>
> 
> **検討中の契約・ライセンス事例と出版社のポリシー**: 契約書と著作権が別であることを説明すべきかもしれません。出版社やライブラリの規約がTDMや自動ダウンロードを制限していることがよくあります。現在の契約例や出版社の規約を確認したいですね。J-STAGEの規約を調べると良さそうです。

**ChatGPT (tool execution)**: <sub>2026-04-22 23:59</sub>

```
{"search_query":[{"q":"JSTOR terms of use automated download text and data mining","domains":["www.jstor.org"]},{"q":"Springer Nature text and data mining policy API terms","domains":["www.springernature.com","www.springernature.com"]},{"q":"J-STAGE 利用規約 自動 ダウンロード","domains":["www.jstage.jst.go.jp"]},{"q":"CiNii Articles 利用規約 自動 ダウンロード","domains":["support.nii.ac.jp","ci.nii.ac.jp"]}],"response_length":"short"}
```

> [!note]- Internal Reasoning
> <sub>2026-04-23 00:00</sub>
> 
> **法的記事の追加情報を検討中**: 法律に関する具体的な言語を直接扱うソースがもう１つ必要かもしれません。文化機関の活用もありかも。オープンアクセスライセンスも触れるべきかもしれませんね。Creative Commonsの情報も調べた方が良さそうです。

**ChatGPT (tool execution)**: <sub>2026-04-23 00:00</sub>

```
{"search_query":[{"q":"Creative Commons BY 4.0 legal code adaptations text and data mining","domains":["creativecommons.org"]},{"q":"CC BY 4.0 日本 データマイニング AI 利用","domains":["creativecommons.org"]},{"q":"arXiv license CC BY text mining policy","domains":["arxiv.org"]}],"response_length":"short"}
```
...
**ChatGPT**: <sub>2026-04-23 00:00</sub>

結論として、あなたの問題設定は大筋で妥当です。ただし、法的評価は「数学研究だから可」「AIだから不可」ではなく、何を複製し、どこへ送り、何を保持し、どの程度原文に依存した出力を返すかで分かれます。日本法では、情報解析目的の利用には著作権法30条の4が広く働き得ますが、享受目的が併存する場合や、権利者の利益を不当に害する場合は外れます。RAG等でも30条の4が常に使えるわけではなく、47条の5の軽微利用にとどまるか、そもそも許諾が要るかが分かれます。文化庁も、既存著作物の創作的表現の全部又は一部を出力させることを目的とした追加学習や、創作的表現の提供自体が主目的の利用は問題になり得ると整理しています。citeturn245263search3turn422259search5turn422259search2turn422259search3

ケースごとに見ると、まず「手元のPCで論文や書籍を解析し、定理名・定義間の関係・索引・ベクトル表現だけを内部生成し、外部に送らず、原文再現もしない」なら、最も安全側です。ここでは著作物の内容を人が読むためではなく情報解析に回しているので、30条の4に乗る余地が大きいです。ただし、情報解析用データベース自体が市場として提供されている類型を代替するような設計だと、ただし書との緊張が出ます。citeturn245263search3turn422259search2turn245263search7

次に「外部APIにPDF全文や章全体を送る」場合です。著作権法上は、送信前の複製だけでなく、外部事業者への入力に伴う利用態様全体を見ます。非享受目的の解析として整理できる余地はありますが、サービス提供者側で再学習・再利用・保持が起こり得る設計だと、法的評価も契約評価も厳しくなります。少なくとも、研究設計としては全文投入を標準フローにしない方がよいです。citeturn245263search3turn422259search5turn422259search3

「RAG用に全文をベクトルDB化する」は中間です。文化庁は、ベクトルDB化のための複製でも、既存著作物の創作的表現を出力させる目的があるなら30条の4は外れると示しています。他方で、所在検索や結果提示に付随する軽微利用なら47条の5が問題になります。つまり、ベクトル化そのものより、何を返す設計かが重要です。回答が原文の代替読書になるほど長い抜粋や、該当箇所の実質的再現を返す設計は危険です。citeturn422259search5turn422259search4turn422259search0

「数行程度のスニペットだけを返し、所在情報と要約を中心にする」なら、かなり整理しやすいです。文化庁の説明でも、検索結果とともにリンク先Webページの一部を数行程度表示する類型は47条の5の適用例として挙げられています。M3Eで文献名、定理名、該当ページ、関連ノードだけを返し、本文はユーザーが原資料を開いて読む設計は、この方向に合います。citeturn422259search0turn422259search4

「原文を要約し、その要約を知識ノード化する」は、通常は全文複製の保存や再配布より安全です。著作権は思想・事実・体系そのものではなく表現を保護するので、表現依存を下げた要約・関係抽出・命題化は有利です。ただし、定義文や証明の特徴的表現をほぼ言い換えなしで残すと、要約ではなく実質的転載に近づきます。ここは“どこまで抽象化したか”が核心です。citeturn245263search8turn422259search3

「定義・定理をそのまま大量保存し、質問に応じて再提示する」設計は危険です。これは情報解析よりも、著作物の表現の蓄積と再提供に寄ります。たとえ内部DBであっても、出力が原文の読書代替になるほど再現的なら、30条の4で守りにくくなります。あなたの問題設定でいう“原文の逐語的再現や実質的な読書代替を回避する必要がある”という部分は、このケースではそのまま正しいです。citeturn422259search2turn422259search5

「オープンアクセス論文やCCライセンス文献を使う」場合は、かなり扱いやすくなります。CC BYのように再利用が広く認められるものは、帰属表示などライセンス条件を守れば、著作権法上の例外に頼らず利用できます。逆にCCでも条件は残るので、出典保持とライセンス記録は必要です。citeturn920116search14turn920116search0

「電子書籍・有料論文・機関契約コンテンツ」は、著作権とは別に契約が強く効きます。ここであなたの問題設定は特に正確です。たとえばJ-STAGEは、私的使用や引用の範囲を超える転載・複製・公衆送信・翻訳・データベース化等には承諾が必要と明示しています。Springer Natureも、TDMを認める場面を分け、商用研究には標準TDM条件や有償APIを案内しています。つまり、法が許す余地があっても、契約上その取得方法や利用方法が制限されることがあります。研究目的だけでは自由になりません。citeturn441333search2turn441333search1turn441333search5

したがって、研究計画としては次の線が最も堅いです。原文は原則ローカル保持、外部AIには全文を送らない、M3Eに入れるのは命題・関係・索引・埋め込み・所在情報・短い適法スニペットに限る、出力は要約と参照案内を中心にし、原文復元モードを持たない、契約付きソースは個別に利用規約を確認し、必要ならOA・CC・自前ノート中心にコーパスを絞る。この設計なら、あなたの研究目的である「数学的知識の体系性を維持しつつ、表現に依存しない構造化知識へ変換する」という筋と、法的・契約的安全性が一致します。citeturn245263search3turn422259search5turn441333search2turn441333search5

あなたの文章を少しだけ法的に引き締めるなら、「外部APIを用いた生成AIに対して著作物を入力する場合、当該著作物の複製・送信・保存・再提供の態様によって適法性が分かれる」「問題となるのは常に入力それ自体ではなく、享受目的の有無、軽微利用の範囲、原文代替性、契約違反の有無である」と書く方が正確です。なお、これは法的助言ではなく研究設計上の整理です。弁護士確認が必要なのは、商用化、外部提供、機関契約資料の大規模処理、全文保持型RAGの4類型です。
