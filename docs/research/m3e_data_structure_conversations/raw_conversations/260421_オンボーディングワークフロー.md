---
title: "オンボーディングワークフロー"
created: 2026-04-21 21:26:51
updated: 2026-04-21 21:28:16
tags:
  - chatgpt
  - conversation
---
...
**User**: <sub>2026-04-21 21:26</sub>

オンボーディングであらゆるローカルソースと自動連係し始めるさいこうのワークフローを考えたいな

> [!abstract]- Tool (file_search)
> <sub>2026-04-21 21:26</sub>
> 
...
> <sub>2026-04-21 21:27</sub>
> 
> 思考時間: 18s

**ChatGPT**: <sub>2026-04-21 21:26</sub>

結論として、オンボーディングで最初から「全ローカルソースを常時自動監視して全部つなぐ」は避けたほうがよいです。M3Eの中核は思考支援であり、基盤実装の負担を抑えつつ有効性を先に検証する方針なので、既存環境を土台にして、取り込みは段階的に自動化する設計が最もよいです。fileciteturn0file4 M3E自体も、AIは提案で人間が確定し、オフラインファーストで、消失バグゼロを最優先に置いています。fileciteturn0file2

最良のワークフローは、初回オンボーディングを4層に分ける形です。第1層は「登録」だけです。ユーザーにローカルソースの種類を選ばせます。たとえば Freeplane の `.mm`、Markdown、PDF 要約置き場、作業フォルダ、日報フォルダ、添付保存先などです。この段階ではまだ自動実行しません。まず場所と形式だけ登録します。M3Eは保存形式や変換境界を明示して扱う設計なので、入口を雑にしないほうが後で壊れません。fileciteturn0file2

第2層は「読取専用の初回スキャン」です。各ソースを読み、M3E内部では正本に直接混ぜず、Flash Intake として受けます。これは既存の AI 利用シナリオとも一致していて、PDF やメモや長文ログのような雑多な素材は、まず Flash に流し込み、そこから必要なものだけ Rapid に上げるのが自然です。fileciteturn0file3 ここで重要なのは、全文同期ではなく、各ソースごとに「カード化された断片」「構造候補」「元ファイルへの参照」を作ることです。つまり、ローカルソースは最初から M3E の正本ではなく、材料プールとして接続します。

第3層は「毎回の運用フロー」です。ここで自動化を入れますが、常時監視ではなく、日課型の手動収集ボタンを中心にします。あなたの現時点の前提にも、メールやDiscordなどの取り込みは「手動収集ボタン」を押す日課型と整理されています。fileciteturn0file3 これはかなり正しいです。理由は3つあります。第一に、ローカルソースはノイズが多い。第二に、常時監視は trust とデバッグが難しい。第三に、M3Eの思想では AI の自動反映ではなく提案差分が重要だからです。fileciteturn0file2 したがって運用は、「朝または作業開始時に収集」「新規候補を Flash に追加」「AI が親候補・重複候補・タイトル整形を提案」「人間が Rapid に昇格」の順がよいです。Flash から Rapid への昇格支援は、親候補提示、フォルダ候補提示、タイトル整形、重複検出を行う仕様と相性がよいです。fileciteturn0file3

第4層は「Deep への接続」です。Rapid に整理された後、必要なスコープだけをコンテクストパッケージ化して外部AIに渡します。M3Eでは、Deep は外部AIに渡す前の設計図として位置づけられており、選択スコープ以下を自然言語化して外部対話に使う方針です。fileciteturn0file2 つまり、ローカルソース→Flash→Rapid→Deep→外部AI という一方向の整流を作るのがよいです。ローカルソースをいきなり Deep に直結させないほうが、思考の主導権を失いません。

これを実際の UX に落とすと、初回オンボーディング画面は次の順で十分です。まず「どのフォルダやアプリを材料源にするか」を選ばせる。次に各ソースに対して「取り込み単位」を選ばせる。全文、要約、見出し単位、ファイル名のみ、更新ファイルのみ、のような粒度です。次に「収集タイミング」を選ばせる。手動のみ、起動時のみ、毎日1回、の3段階で足ります。最後に「M3Eへの入り口」を選ばせる。既定は Flash 固定でよいです。これで十分に強いです。

設計原則としては、同期ではなく変換、正本化ではなく参照、常時監視ではなく収集トリガー、全面自動分類ではなく昇格支援、がよいです。これは、scope 境界・alias 参照・AI提案差分という M3E の仕様とも整合します。実体は一意に保ち、他所から見せたいときは参照で扱うのが原則です。fileciteturn0file2 したがって、各ローカルソースは M3E 内で「source node」または「import card」として参照し、内容を本体ツリーに溶かすのは昇格時だけにすべきです。

要するに、さいこうのワークフローはこうです。初回はローカルソースを登録する。日常では収集ボタンで差分だけ吸い上げる。吸い上げた内容は全部 Flash に入れる。AI は整理候補だけを出す。人間が Rapid に昇格する。必要な subtree だけ Deep 化して外部AIに渡す。これが最も壊れにくく、実装負担も小さく、M3Eの価値も最も早く検証できます。fileciteturn0file4

参考として、今回の基盤文書は [M3E仕様設計書](sandbox:/mnt/data/M3E仕様設計書.md)、[M3E仕様2](sandbox:/mnt/data/M3E仕様2.md)、[freeplane利用について](sandbox:/mnt/data/freeplane利用について.txt)、[TheDesign](sandbox:/mnt/data/TheDesign.md) です。
