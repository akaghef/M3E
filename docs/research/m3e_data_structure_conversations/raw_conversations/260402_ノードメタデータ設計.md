---
title: "ノードメタデータ設計"
created: 2026-04-02 03:01:50
updated: 2026-04-02 03:02:09
tags:
  - chatgpt
  - conversation
---
...
**User**: <sub>2026-04-02 03:01</sub>

ノードメタデータ列挙って設計資料にある？

> [!abstract]- Tool (file_search)
> <sub>2026-04-02 03:01</sub>
> 
...
**ChatGPT**: <sub>2026-04-02 03:01</sub>

結論：**「ノードメタデータ」という名前での網羅的列挙は存在しないが、実質的な定義は分散して記述されている。**

具体的には次の通り。

---

### 1. 実質的なメタデータ定義（Node属性）
仕様書では「Nodeの必須属性」として列挙されている：
...
これは実質的にノードメタデータのコア定義になっている。  
fileciteturn0file2

---

### 2. メタデータに含まれるが分散している要素
明示的に「メタデータ」とは書かれていないが、以下も同種の情報：

- ViewState（選択、ドラッグ位置など）  
- Undo用情報（fromParent, indexなど）  
- スキーマバージョン  
- 差分ログ  

これらは別章・別責務として記述されている。  
fileciteturn0file0  
fileciteturn0file3
...
### 3. 設計上の特徴
- **構造状態とUI状態を分離している**
- メタデータを「Nodeに閉じて定義していない」
- Command / ViewState / Log に分割して管理
...
### 結論
- コアメタデータは「Node必須属性」として存在  
- ただし体系的な「メタデータ一覧」は未整備  
- 設計的には意図的に分散管理されている状態
