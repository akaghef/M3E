...
project: SelfDrive
date: 2026-04-20
status: active (reopened 2026-04-21 per plan2.md)
owner: akaghef
related: plan.md
---
...
PJ03 では一時期、`tree-like M3E` の発想を system diagram / graph runtime にまで持ち込みすぎた。
そのため、本来は **tree と相補的に並ぶ non-tree runtime** を取り込む試みが、
`tree への projection` や `scope 表示` の話に読み替えられた。

この点は明確に区別する:

- knowledge tree と system diagram は根本的に性質が違う
- system diagram は tree の派生物ではない
- LangGraph 的 runtime を目指すなら graph-first で考える
- tree / scope は必要なら projection や表示面として使うが、runtime の正本ではない

関連:

- active plan: [plan2.md](plan2.md)
- demo / projection plan: [plan3.md](plan3.md)
- warning memo: [idea/non_tree_graph_runtime_warning.md](idea/non_tree_graph_runtime_warning.md)
...
- tree と並立する non-tree graph runtime の本格導入（LangGraph または同等 graph-first runtime）
...
- 2026-04-21: Phase 2 完了（T-2-1..T-2-5）、Evaluator 4/4 pass、dogfood_run_02 で 5 停止カテゴリ全 runtime 観察 → Gate 3 承認
- 2026-04-21: Phase 3 kickoff → T-3-1..T-3-4 完了、workflow_scope_projector 実装、dogfood_run_03 で PJ03 自身を投影、retrospective 作成
- 2026-04-21: **PJ03 完了（全 25 task done、retrospective.md / Future Work 整理済）**
- 2026-04-21: **PJ03 reopen（Qn5_premature_done_declaration）** — akaghef が plan2.md で再定義。Plan 1 は「graph runtime の基礎工事」まで、Plan 2 で LangGraph 的実体を成立させる。active plan = plan2.md、新 Phase A–D を T-4-*..T-7-* として起票
