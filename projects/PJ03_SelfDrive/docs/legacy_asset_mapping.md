...
- **referenced by**: T-1-3 runner 実装（writeback 先の決定）、T-1-5 checkpoint 実装
...
|---|---|---|---|---|
| `tasks.yaml` | `projects/PJ03_SelfDrive/tasks.yaml` の `status` / `round` / `last_feedback` / `blocker` | 9 state すべて（永続化先） | 直接は triggerしない。runner が読み書き | **primary checkpoint**: 1 task の最新 state はここで確定 |
...
| `ScheduleWakeup` | `.claude/skills/` 経由の native API | `sleeping` | E08 (in_progress → sleeping), E09 (sleeping → ready) | 時間ベース checkpoint（次回起動時に runner が拾う） |
| `CronCreate` | 同上、scheduled trigger | `sleeping`（長周期 or 繰返し） | E08, E09 | ScheduleWakeup と相補（ScheduleWakeup は one-shot / Cron は繰返し） |
| `SessionStart` hook | `.claude/scripts/hooks/session-start.sh` 相当 | — (read-only、runner 復元) | 起動時に in_progress task を検出して resume へ誘導 | **resume entry point**: resume-cheatsheet.md + tasks.yaml を読む強制 |
...
| `sub-pj-do` skill | `.claude/skills/sub-pj-do/` | 決定論的ループの指示文字列 | 現状は指示文字列経由で全 edge を trigger（engine 化で runner に移譲） | （skill 自体は checkpoint ではない、runner のラッパー） |
...
| asset | 現状 | 必要性 |
|---|---|---|
| `Progress Board` (map scope) | 人間 read-only、runner 未接続 | Phase 3 で engine ↔ map の writeback を設計 |
...
| `plan.md 進捗ログ` | 人間記述 + Claude 追記 | done 時に runner が自動追記する導線は Phase 2 以降 |
...
- T-1-3 runner: writeback 対象は `tasks.yaml`（primary）+ `resume-cheatsheet.md`（human summary）
