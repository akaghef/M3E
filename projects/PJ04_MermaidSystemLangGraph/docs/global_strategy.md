...
> 実用検証の縦糸は **Template-first PJv34 Rebuild**: System Block Template を実装し、AI がテンプレから PJv34 Weekly Review system を再構築し、Run まで通す。
...
- **量産する**: LangGraph pattern を System Block Template として持ち、AI が template insert + contract fill で system を再構築できるようにする
...
### 4.2.1 Template-first PJv34 Rebuild

PJ04 の実用検証は、抽象的な canonical sample だけではなく **PJv34 Weekly Review** を使う。

手順:

1. LangGraph pattern を System Block Template として定義する
2. M3E で template insert → Contract Tree 初期化を実装する
3. AI に `llm.generate_doc.subsystem` / `io.load_local_folder` / `io.write_artifact` などの template から PJv34 system を再構築させる
4. 再構築された Authoring Map から GraphSpec を compile する
5. 既存の PJv34 weekly review loop と同等の Run を通す
...
- AI がゼロから node 属性を手書きせず、template の contract 初期値を埋めて system を作る
...
TPL-0 ──► TPL-1 ──► TPL-2
  │        │        │
  │        │        └──► T-B-1/T-C-1 acceptance (PJv34 Run)
  │        └──────────► T-A-1 (template-derived GraphSpec determinism)
  └───────────────────► CX-0 (template kind / channel badge)
```
...
| **W3 並行** | TPL-1 | TPL | template insert + Contract Tree 初期化 | `llm.generate_doc.subsystem` を置ける |
| **W4** | **G2**: T-B-1 | EXEC | bridge MVP (`bridge.py` / `registry.py` + TS client) | invoke 往復、`introspect` / `fetch_source` IPC |
| **W4-5** | TPL-2 | TPL | AI が PJv34 を template から再構築して Run | PJv34 Weekly Review equivalent run |
