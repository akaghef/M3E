...
> **TL;DR**: M3E を **LangGraph 系システムの協働 authoring 環境** にする。**5 層** (L1 Outer / L2 Inner / L3 Spec / L4 Bridge / L5 Runtime) を **4 トラック並走 + 2 ゲート直列** で組み上げる。
...
> 最終目的は **6-step Collaboration Stance** (Sketch → Fill → Review → Tune → Run → Iterate) が map 内で完結する状態。
> **authoring は骨格のみ、詳細は L2 Inner、surface にはエラーだけ**。この原則を破ったらトラックを止めて戻る。

---

## 1. 大域的な 1 行

**M3E を LangGraph 系システムの協働 authoring 環境とし、L1 Outer (人間の骨格) → L2 Inner (AI の詳細) → L3 Spec (derived) → L4 Bridge (subprocess) → L5 Runtime (本家) の 5 段を、map を壊さず 4 トラック並走で成立させる。**

- **作らない**: 独自 runtime / 独自 graph library / 独自 checkpoint fork (N1-N10 参照)
- **守る**: map は唯一の authoring 正本、node = scope 統一 (I12)、AI-fill は L2 限定 (I13)
...
| **I-G2** | surface (L1 Outer) には **負 (error/warning) しか置かない**、詳細は L2 Inner | I8 / authoring minimalism |
...
| **I-G10** | AI-fill は L2 Inner のみ。L1 Outer は常に人間の担当 | I13 |
...
```
① Sketch (人間)  L1 Outer に骨格: agent 名 + 主要 edge
② Fill   (AI)    L2 Inner を埋める: prompt / source / schema / channels / 子 tool
③ Review (人間)  [ で scope に入り L2 を確認
④ Tune   (人間)  L2 の inline data を直接編集 or AI に局所再生成指示
⑤ Run    (M3E)   compile → L4 Bridge → L5 Runtime、live-view が L2 にストリーム
...
- **DATA** は ①②③ (channel 宣言を L2 に置ける / 赤黄バッジで未定義を検出) を成立させる
...
| **W2 並行** | CX-0 | VIEW | L2 (kind icon + channels badge) | bridge 無しで box が役割を示す |
...
1. **6-step Collaboration Stance が 1 周回る** — 人間が L1 Outer に agent scope を描き、AI-fill で L2 Inner が埋まり、compile → run → live-view が L2 に戻るループが 1 本成立 (§4.3)
