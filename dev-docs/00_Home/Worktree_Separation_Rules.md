...
- `codex1`: `dev-visual`（visual 関連）
- `codex2`: `dev-data`（model / controller 関連）
- `akaghef`: 全体レビューと指示
...
- `C:/Users/Akaghef/dev/M3E-dev-visual` -> `dev-visual`
- `C:/Users/Akaghef/dev/M3E-dev-data` -> `dev-data`
...
- `dev-visual` と `dev-data` の成果は `dev-beta` に統合する。
...
```bash
git fetch origin
git checkout dev-visual   # codex2 の場合は dev-data
...
git worktree add -b dev-visual ../M3E-dev-visual dev-beta
```
