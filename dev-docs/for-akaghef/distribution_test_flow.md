...
無人インストール設定画面:
- ユーザー名: `m3etest`
- パスワード: 任意（`vm_test.bat` の `GUEST_PASS` と合わせること）
...
確認: ホスト側から以下が通ればOK:
```powershell
& "C:\Program Files\Oracle\VirtualBox\VBoxManage.exe" guestcontrol "M3E-Test" run --username "m3etest" --password "<パスワード>" --exe "C:\Windows\System32\cmd.exe" -- cmd /c "echo hello"
```
...
セットアップ時にファイアウォールルール（Node.js のポート許可）を自動追加するため、
`m3etest` ユーザーに管理者権限が必要。VM 内で管理者コマンドプロンプトを開いて:

```bat
net localgroup Administrators m3etest /add
```

これをやらないと、毎回 Node.js 起動時にファイアウォール許可ダイアログが出て自動テストが止まる。
