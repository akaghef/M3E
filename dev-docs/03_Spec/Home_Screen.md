...
M3E 起動時の「玄関」となる画面。ユーザーが複数のドキュメントを管理するための**ドキュメント一覧ビュー**として機能する。viewer は「1つのドキュメントを開いて編集する場所」、Home は「どのドキュメントに入るか選ぶ場所」。

## 2. 基本方針

- **Home = ドキュメント一覧ビュー**。scope tree や tag 一覧は補助であって主役ではない。
- `/` アクセスで Home が開く。
...
Miro のダッシュボードに近いイメージ。上から「新規作成」「ピン止め」「全ドキュメント」「Archived」の縦並び。
...
| カード | 動作 |
|---|---|
| **Blank** | `POST /api/docs/new` → 空 doc を作り viewer へ遷移 |
| **Obsidian Vault** | vault ディレクトリを選択 → そのディレクトリを source とする tight-coupled doc を作成（§4.2.1） |
| **Import .md / JSON** | ファイル選択 → パースして doc 化（既存の md_reader を配線） |

#### 4.2.1 Obsidian Vault との tight coupling
- vault ディレクトリを選ぶと、その中の `.md` を node として取り込み、以後は双方向同期。
- doc の `source` 属性に vault path を持たせる（`source: { kind: "obsidian", path: "..." }`）。
- 一覧カードの 2 行目に `obsidian: <vault path>` を副表示。
- Phase 1 では UI の枠だけ用意し、ボタンは "Coming soon" 無効化でも可。ただし**設計はこの仕様書で確定させる**。

### 4.3 Pinned（スター付き）
- ユーザーが星マークを付けた doc を横並びカードで上部に表示。
- 各カードから直接 viewer へ遷移。
...
### 4.4 Documents（全一覧）
**全ドキュメント**を更新日時降順で並べる（ピンの有無に関係なく全件）。

各行（Document Card）:
- 左端: `☆/★` ピントグル
- アイコン（📄 or source別に 📘=obsidian など）
- **ドキュメントラベル**（クリックで viewer へ）
...
| Tags セクション | **Phase 2 以降で再検討**。代わりに各 doc カードに tags を表示 |
...
| `[open]` | 同上 |
| `[+ New Document]` | `POST /api/docs/new` → 返ってきた id で viewer へ遷移 |
...
- `POST /api/docs/import-vault` — Obsidian vault ディレクトリを source として新 doc 作成
...
- 既存 doc を後から vault に bind する動線: `[⋯] > bind to vault` を**用意する**
- Create new カード: 3 枚すべて実装し有効化（§9 のとおり Obsidian/Import も同時に配線）

## 11. 本仕様の範囲外

- Obsidian vault との同期粒度・データマッピングは [Obsidian_Vault_Integration.md](./Obsidian_Vault_Integration.md) の管轄。Home 側では「vault を bind する入口」「bind 中の doc を一覧表示」「unbind する動線」のみを規定する。
