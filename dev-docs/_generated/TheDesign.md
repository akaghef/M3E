...
- `parentId: string | null`
- `children: string[]`（順序意味あり。参照ノードは常に空）
...
ノードは **実体ノード（entity）** と **参照ノード（reference）** に大別される。
この分類は nodeType が増えても変わらない不変の軸である。

- **実体ノード** (`text`, `image`, `folder`): 固有の内容を持ち正本が 1 箇所。子を持てる。scope に所属する。alias の target になれる
- **参照ノード** (`alias`): 実体への参照窓。固有内容なし。children 常に空（leaf 強制）。参照→参照の連鎖禁止

### 2.3 型別 MUST

- `text`（実体）: `content.text: string`
- `image`（実体）: `content.imageRef: string`
- `folder`（実体）: `content.title: string`
- `alias`（参照）: `content.targetNodeId: string`
