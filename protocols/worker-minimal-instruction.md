...
5. `protocols/persistent-rule-change-protocol.md` when the user instruction contains `!!!` / `！！！` or asks for recurrence prevention after an agent failure
...
2. Update at least one canonical rule source such as root `AGENTS.md`, `protocols/`, `protocols/contracts/`, `agent_instructions/skills_canonical/`, or checked-in hook/guard scripts.
