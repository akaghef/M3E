...
test("createConflictBackup saves a backup and returns entry metadata", () => {
  ...
  const entry = createConflictBackup(tempDataDir, "map-1", state, "cloud-sync-pull");

  expect(entry.backupId).toBeTruthy();
  expect(entry.mapId).toBe("map-1");
...
test("listConflictBackups returns saved backups sorted newest first", () => {
  ...
  const entry1 = createConflictBackup(tempDataDir, "map-1", state1, "reason-1");
  const entry2 = createConflictBackup(tempDataDir, "map-1", state2, "reason-2");

  const list = listConflictBackups(tempDataDir, "map-1");
...
test("listConflictBackups filters by mapId", () => {
  createConflictBackup(tempDataDir, "map-a", createState("A"), "r1");
  createConflictBackup(tempDataDir, "map-b", createState("B"), "r2");

  const listA = listConflictBackups(tempDataDir, "map-a");
  expect(listA.length).toBe(1);
  expect(listA[0].mapId).toBe("map-a");

  const listB = listConflictBackups(tempDataDir, "map-b");
  expect(listB.length).toBe(1);
  expect(listB[0].mapId).toBe("map-b");
});

test("getConflictBackup returns full backup with state", () => {
  const state = createState("GetTest");
  const entry = createConflictBackup(tempDataDir, "map-1", state, "test-reason");

  const full = getConflictBackup(tempDataDir, "map-1", entry.backupId);
  expect(full).toBeTruthy();
  expect(full.version).toBe(1);
  expect(full.backupId).toBe(entry.backupId);
  expect(full.mapId).toBe("map-1");
...
test("getConflictBackup returns null for nonexistent backup", () => {
  const full = getConflictBackup(tempDataDir, "map-1", "nonexistent-id");
  expect(full).toBe(null);
});

test("deleteConflictBackup removes the backup file", () => {
  const entry = createConflictBackup(tempDataDir, "map-1", createState("Del"), "r");

  const deleted = deleteConflictBackup(tempDataDir, "map-1", entry.backupId);
...
test("deleteConflictBackup returns false for nonexistent backup", () => {
  const deleted = deleteConflictBackup(tempDataDir, "map-1", "nonexistent");
  expect(deleted).toBe(false);
});

test("createConflictBackup prunes backups beyond limit of 10", () => {
  for (let i = 0; i < 12; i++) {
    createConflictBackup(tempDataDir, "map-prune", createState(`N${i}`), `r${i}`);
  }

  const list = listConflictBackups(tempDataDir, "map-prune");
