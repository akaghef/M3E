...
async function canvasPointUnderClient(page, clientX, clientY) {
  return page.evaluate(({ clientX, clientY }) => {
    const board = document.querySelector("#board");
    const canvas = document.querySelector("#canvas");
    if (!board || !canvas) {
      throw new Error("Viewer board/canvas not found.");
    }
    const boardRect = board.getBoundingClientRect();
    const inlineTransform = canvas.style.transform || "";
    const transformNumber = "[-+]?\\d*\\.?\\d+(?:e[-+]?\\d+)?";
    const inlineMatch = inlineTransform.match(new RegExp(
      `translate\\((${transformNumber})px,\\s*(${transformNumber})px\\)\\s+scale\\((${transformNumber})\\)`,
      "i",
    ));
    if (inlineMatch) {
      const cameraX = Number(inlineMatch[1]);
      const cameraY = Number(inlineMatch[2]);
      const zoom = Number(inlineMatch[3]);
      return {
        x: (clientX - boardRect.left - cameraX) / zoom,
        y: (clientY - boardRect.top - cameraY) / zoom,
        zoom,
      };
    }
    const transform = getComputedStyle(canvas).transform;
    const matrix = new DOMMatrixReadOnly(transform === "none" ? undefined : transform);
    const point = new DOMPoint(clientX - boardRect.left, clientY - boardRect.top).matrixTransform(matrix.inverse());
    return { x: point.x, y: point.y, zoom: matrix.a || 1 };
  }, { clientX, clientY });
}

async function selectedVisualDeltaFromBoardCenter(page) {
  return page.evaluate(() => {
    const board = document.querySelector("#board");
    const selected = document.querySelector(
      "circle.scatter-node-circle.primary-selected, path.node-shape.primary-selected, rect.root-box.primary-selected"
    );
    if (!board || !selected) {
    ...
    return {
      x: selectedRect.left + selectedRect.width / 2 - (boardRect.left + boardRect.width / 2),
      y: selectedRect.top + selectedRect.height / 2 - (boardRect.top + boardRect.height / 2),
    };
  });
}
...
async function resetProgressiveMeasureCount(page) {
  await page.evaluate(() => {
    window.__m3eProgressiveMeasureEdgesCount = 0;
  });
}

async function progressiveMeasureCount(page) {
  return page.evaluate(() => window.__m3eProgressiveMeasureEdgesCount || 0);
}
...
test.describe("Zoom shortcuts (-, =, 0)", () => {
  ...
  test("Ctrl+wheel keeps the canvas point under the pointer stable", async ({ page }) => {
    await launchViewer(page);
    await focusBoard(page);
    ...
    const anchor = {
      x: board.x + board.width * 0.37,
      y: board.y + board.height * 0.42,
    };
    const before = await canvasPointUnderClient(page, anchor.x, anchor.y);
    ...
    const after = await canvasPointUnderClient(page, anchor.x, anchor.y);
    const oneScreenPixelInCanvasUnits = 1 / Math.max(1, after.zoom);
    expect(Math.abs(after.x - before.x)).toBeLessThanOrEqual(oneScreenPixelInCanvasUnits);
    expect(Math.abs(after.y - before.y)).toBeLessThanOrEqual(oneScreenPixelInCanvasUnits);
  });

  test("drag pan moves content without closed progressive navigation measurement", async ({ page }) => {
    await launchViewer(page);
    await focusBoard(page);
    await waitForRender(page, 100);
    await resetProgressiveMeasureCount(page);

    const before = await selectedVisualCenter(page);
    const board = await page.locator("#board").boundingBox();
    expect(board).toBeTruthy();
    const start = {
      x: board.x + board.width * 0.28,
      y: board.y + board.height * 0.72,
    };
    const delta = { x: 92, y: -48 };

    await page.mouse.move(start.x, start.y);
    await page.mouse.down();
    await page.mouse.move(start.x + delta.x, start.y + delta.y, { steps: 8 });
    await page.mouse.up();
    await waitForRender(page, 350);

    const after = await selectedVisualCenter(page);
    expect(Math.abs((after.x - before.x) - delta.x)).toBeLessThan(1.5);
    expect(Math.abs((after.y - before.y) - delta.y)).toBeLessThan(1.5);
    expect(await progressiveMeasureCount(page)).toBe(0);
  });

  test("open progressive navigation coalesces viewport measurements", async ({ page }) => {
    await launchViewer(page);
    const navButton = page.locator('button[aria-label="[GUI] navigation root"]');
    ...
    await waitForRender(page, 80);

    const count = await progressiveMeasureCount(page);
    expect(count).toBeGreaterThan(0);
    expect(count).toBeLessThanOrEqual(1);
  });

  test("scatter mode selects a rendered node before focus centering", async ({ page }) => {
    await launchViewer(page);
    await focusBoard(page);

    await page.evaluate(() => {
      document.querySelector("#view-scatter")?.dispatchEvent(new MouseEvent("click", { bubbles: true }));
    });
    await waitForRender(page, 700);
    await expectMetaContains(page, "selected: Child A");
    await expect(page.locator("circle.scatter-node-circle.primary-selected")).toBeVisible();

    await pressKey(page, "Alt+v");
    await waitForRender(page, 700);
    const delta = await selectedVisualDeltaFromBoardCenter(page);
    expect(Math.abs(delta.x)).toBeLessThan(1.5);
    expect(Math.abs(delta.y)).toBeLessThan(1.5);
  });
});
