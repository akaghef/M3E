...
## 2026-05-02 Secretary handoff: V4 product experience correction

status: in-progress
summary: Akaghef rejected the API-only interpretation as not matching intent. Executive added `docs/03_Spec/V4_Product_Experience_Blueprint.md` as the product-experience source of truth. The intended flow is now Source Tray, Mapify Draft Review, Discussion Surface, Obsidian Live Workspace, Conflict Workbench, GitHub Safety Boundary, and a two-pane report replay using actual M3E screens. This explicitly distinguishes current API capability from the GUI/product experience still required.
implementation: Added a first GUI slice in Beta viewer: `V4 Workbench` panel. It shows Flash draft list/count, Obsidian live status/path, current conflict panel state, GitHub safety-boundary status, and selected node source metadata. It also adds `Sticky` and `Decision` buttons that create discussion-attributed nodes under the selected node. Added a minimal Mapify GUI path: source text textarea, `Create Draft`, and `Apply Latest Pending`.
verification: `npm --prefix beta run build` passed. Restarted port 4173. Headless Playwright opened the V4 demo map, clicked `V4`, confirmed panel visibility/title, Flash status, Vault status, selected source text, and successfully added a V4 sticky node. A second Playwright run pasted markdown source, created a Flash / Mapify draft, applied the latest pending draft, and confirmed `GUI Mapify Test` appeared in the rendered page.
remaining: Source Tray with PDF binary extraction, persistent Mapify draft review UI, node-level accept/reject editing, Discussion surface layout, Obsidian changed-file preview/merge import, 3-column Conflict Workbench, and GitHub publish proposal/secret lock UI remain to be implemented.
